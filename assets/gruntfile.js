...
module.exports = function(grunt) {
	...
	grunt.initConfig({
		...
		sass 	: {
			style 	: {
				...
				files: {
				    ...
				    '<%= dirs.publicCss %>/print.min.css': 'sass/print.scss',
				}
			},
			devStyle 	: {
				options : {
		...
		concat 	: {
			...
			doInc 		: {
				src 	: [
					// '<%= dirs.jsLib %>/jquery.js', // unload the WP JS - and don't use any WP plugins / Fuck/
					// '<%= dirs.jsLib %>/underscore.js', // Load if you need these util
		...
		/*
			...
			at the end of the file), or are included at the begining of
			the  build tasks.
		*/
	...
	*/
	...
	grunt.registerTask('admin' , ['concurrent:adminPrep', 'uglify:adminjs', 'focus:admin']); // 'sass:admin', 'concat:adminBuild', 'uglify:adminjs', 'focus:admin'
